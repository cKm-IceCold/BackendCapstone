...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action == 'login' or self.action == 'social_login':
            return [permissions.AllowAny()]
        return [permissions.IsAuthenticated()]

    def perform_create(self, serializer):
        """
        This method is called during the standard POST /api/users/ request.
        The print statement helps verify if React data is reaching the server.
        """
    ...
    def login(self, request):
        """
        Custom action for standard Email/Password login.
        URL: /api/users/login/
        """
        email = request.data.get('email')
        password = request.data.get('password')

        # authenticate uses 'email' because of your USERNAME_FIELD setting
    ...
    def social_login(self, request):
        """
        Action for OAuth/Social login based only on email.
        """
        email = request.data.get('email')
        if not email:
    ...
    def me(self, request):
        """
        Returns current logged-in user data based on the Token header.
        """
